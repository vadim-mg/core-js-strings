...
function repeatString(str, times) {
  return String(str).repeat(times >= 0 ? times : 0);
}
...
function countVowels(str) {
  ...
  return Array.from(str).reduce((acc, val) => {
    return acc + volves.includes(val);
  }, 0);
}
...
function isPalindrome(str) {
  const lcStr = str.toLowerCase().replace(/[, ?!]/gm, '');
  return Array.from(lcStr).reverse().join('') === lcStr;
}
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((val) => Array.from(val).reverse().join(''))
    .join(' ');
}
